...
const Login = () => {
  ...
  const handleOnSubmit = async (e) => {
    ...
    try {
      ...
      //dispatch(setProgram(response.data.data.departments[0].departmentName)); // When the use logs in the program should be selected not automatically.
      localStorage.setItem("userData", JSON.stringify(response.data.data));
      localStorage.setItem("token", response.data.token);
      // localStorage.setItem(
      //   "program",
      //   response.data.data.departments[0].departmentName
      // );
